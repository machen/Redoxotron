...
def open_port(ser):
    ...
    try:
        ...
        if ser.is_open==False:
            ...
            if testread != testread2 and testread2 !='':
                print("AMC0 sucessfully opened, but device may still be collecting data from previous run...sending abort command and i/o flush")
                abort_potentiostat(ser)
                time.sleep(0.5)
                checkser=ser.is_open
                
                if checkser==True:
                    print("buffer erase seemed to work...continuing to data collection")
                    return ser

            else:
                
                print("AMC0 succesfully opened. writing experimental parameters")
                write_params(ser)
                return ser
...
def alt_port(ser):
    ...
    try:
        ...
        if ser.is_open==False:
            ...
            testread2=ser.read(10)
            if testread != testread2 and testread2 !='':
                print("AMC1 sucessfully opened, but device may still be collecting data from previous run...sending abort command and i/o flush")
                abort_potentiostat(ser)
                time.sleep(0.5)
                checkser=ser.is_open
                ...
                print("AMC1 succesfully opened. writing experimental parameters")
                write_params(ser)
                return ser
        ...
        print("\ncould not open AMC1, retrying AMC0 (2nd attempt), and staying in loop....\n")
        if ser.is_open==True:
            ser.close()
        open_port()
        time.sleep(1)
...
def write_params(ser):
        ...
        ser.write(b'!0\n'); time.sleep(1)
        ser.write(b'V\n'); time.sleep(1)
        ser.write(b'!10\n'); time.sleep(1); ser.write(b'EA%d 03 1 \n'%adc_gain);time.sleep(1) #if changing gain, check number of characters to send
        ser.write(b'!6\n'); time.sleep(1); ser.write(b'EG%d 0 \n'%ex_dac_gain);time.sleep(1) 
...
def abort_potentiostat(ser):      #aborts data collection without closing port
        ...
        for i in range(20):
            ...
            if b'#' in data:
                ...
                print("Potentiostat program successfully aborted, flushing i/o buffer")
                time.sleep(2)
                ser.flushOutput()
                ser.flushInput()
                time.sleep(1)
                ...
                print("waiting for potentiostat to stop measurement. Timeout after 25 datapoints. current iteration: %d"%i)
                time.sleep(1)
        ...
        count+=1
        if count == 10:
            print("number of abort attempts greater than 10, attempting port reset")
            open_port(ser)
...
def data_collection(loopnumber, start_time, ser, refresh_time, looptime, exptime, collection_countdown):
    ...
    try:
        ...
        for line in ser:
            ...
            if data.startswith(b'B'): # note: dstat returns data in binary as hexadecimal, form \xhh (where hh = 2-value hex)
            ...
            if time_now >=looptime:
                ...
                print("\npotentiostat refresh, breaking data collection loop"); time.sleep(1)
...
def main():
    ...
    while True:
        ...
        try:
            ...
            if use_port_refresh==False and use_abort==False and init_flag==0:
